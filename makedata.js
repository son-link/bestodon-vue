...
const lists = ["games", "apps", "distros", "communities", "dev"];
          ...
          allResp.forEach((resp) => {
            if (!resp.data.error) {
              ...
              const arr = {
                ...
                acct:
                  domainAPI.hostname == domain.hostname
                    ? `${data.acct}@${domainAPI.hostname}`
                    : data.acct,
...
const getData = async () => {
  ...
  lists.forEach(async (list) => {
    ...
    /*const content = fs.readFileSync(`_lists/apps.yml`, 'utf-8').trim();
    const accts = YAML.parse(content);
    console.log(accts)
    return;
    */
    fs.readFileSync(`_lists/${list}`, "utf-8")
      .split("\n")
      .forEach((line) => {
        line = line.trim();
    ...
    /*
    accts.forEach( acct => {
      if (!prevAccts.includes(acct.acct)) accounts.push(line);
    })*/
    ...
    await Promise.all(promises).then((data) => {
      ...
      const writeAllListStream = fs.createWriteStream(
        `./assets/lists/all.json`
      );
      writeAllListStream.write(JSON.stringify(allContent, null, 2));

      if (accts2merge[list].length > 0) {
        let accts = [];
        const writeListStream = fs.createWriteStream(
          `./assets/lists/${list}.json`
        );
...
const listData = (list) => {
  try {
    const content = fs
      .readFileSync(`assets/lists/${list}.json`, "utf-8")
      .trim();
